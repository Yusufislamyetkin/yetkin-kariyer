...
export default function ElegantTemplate({ data }: { data: CVData }) {
  return (
    <div className="bg-white text-gray-900 break-words overflow-hidden" style={{ fontFamily: 'Garamond, serif', height: '100%', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div className="max-w-4xl mx-auto p-4 flex-1 overflow-hidden" style={{ display: 'flex', flexDirection: 'column' }}>
        <div className="text-center border-b-2 border-gray-400 pb-3 mb-2 flex-shrink-0">
          <h1 className="text-2xl font-light mb-1 text-gray-900" style={{ letterSpacing: '2px' }}>{data.personalInfo.name || "Ad Soyad"}</h1>
          <div className="text-xs text-gray-600 space-y-1" style={{ letterSpacing: '0.5px' }}>
        ...
        <div className="flex-1 overflow-y-auto" style={{ minHeight: 0 }}>
          {data.summary && (
            <section className="mb-2 text-center flex-shrink-0">
              <div className="w-12 h-px bg-gray-400 mx-auto mb-1"></div>
              <p className="text-gray-700 leading-tight break-words whitespace-pre-line italic text-sm line-clamp-3">{data.summary}</p>
              <div className="w-12 h-px bg-gray-400 mx-auto mt-1"></div>
            </section>
          )}
          {data.experience.length > 0 && (
            <section className="mb-2">
              <h2 className="text-lg font-light mb-2 text-gray-900 uppercase tracking-widest text-center border-b border-gray-300 pb-1">İş Deneyimi</h2>
              <div className="space-y-1.5">
                {data.experience.map((exp, i) => (
                  <div key={i} className="text-center">
                    <h3 className="text-base font-light text-gray-900 mb-0.5">{exp.position}</h3>
                    <p className="text-gray-600 italic text-xs mb-0.5">{exp.company}</p>
                    <p className="text-xs text-gray-500 mb-1">{exp.startDate} - {exp.current ? "Devam ediyor" : exp.endDate}</p>
                    {exp.description && <p className="text-gray-700 mt-1 break-words whitespace-pre-line text-xs max-w-2xl mx-auto line-clamp-2">{exp.description}</p>}
                    {i < data.experience.length - 1 && <div className="w-8 h-px bg-gray-300 mx-auto mt-1.5"></div>}
                  </div>
                ))}
              </div>
            </section>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {data.education.length > 0 && (
              <section>
                <h2 className="text-base font-light mb-1 text-gray-900 uppercase tracking-widest text-center border-b border-gray-300 pb-1">Eğitim</h2>
            ...
            {data.skills.length > 0 && (
              <section>
                <h2 className="text-base font-light mb-1 text-gray-900 uppercase tracking-widest text-center border-b border-gray-300 pb-1">Beceriler</h2>
