  ...
  return (
    <div className="bg-white text-gray-900 break-words overflow-hidden" style={{ height: '100%', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      <div className="max-w-6xl mx-auto flex-1 overflow-hidden" style={{ display: 'flex', flexDirection: 'column' }}>
        <div className="bg-gradient-to-r from-purple-800 via-pink-600 to-red-600 text-white p-4 relative overflow-hidden flex-shrink-0">
          ...
          <div className="relative">
            <h1 className="text-2xl font-bold mb-1" style={{ fontFamily: 'serif', letterSpacing: '1.5px' }}>{data.personalInfo.name || "Ad Soyad"}</h1>
          ...
          <div className="grid md:grid-cols-2 gap-4 flex-1 overflow-hidden" style={{ minHeight: 0 }}>
            <div className="space-y-2 overflow-y-auto" style={{ maxHeight: '100%' }}>
              {data.experience.length > 0 && (
                <section>
                  <div className="h-1 w-16 bg-gradient-to-r from-purple-600 to-pink-600 mb-1"></div>
                  <h2 className="text-lg font-bold mb-2 text-gray-900" style={{ fontFamily: 'serif' }}>Deneyim</h2>
                  <div className="space-y-1.5">
                    {data.experience.map((exp, i) => (
                      <div key={i} className="relative pl-3 border-l-4 border-purple-600">
              ...
              {data.projects.length > 0 && (
                <section>
                  <div className="h-1 w-16 bg-gradient-to-r from-purple-600 to-pink-600 mb-1"></div>
                  <h2 className="text-lg font-bold mb-2 text-gray-900" style={{ fontFamily: 'serif' }}>Projeler</h2>
                  <div className="space-y-1.5">
            ...
            <div className="space-y-2 overflow-y-auto" style={{ maxHeight: '100%' }}>
              {data.skills.length > 0 && (
                <section>
                  <div className="h-1 w-16 bg-gradient-to-r from-purple-600 to-pink-600 mb-1"></div>
                  <h2 className="text-lg font-bold mb-2 text-gray-900" style={{ fontFamily: 'serif' }}>Beceriler</h2>
                  <div className="flex flex-wrap gap-1">
                    {data.skills.map((skill, i) => (
                      <span key={i} className="px-2 py-1 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-full text-xs font-semibold">{skill}</span>
                    ))}
                  </div>
                </section>
              )}
              {data.education.length > 0 && (
                <section>
                  <div className="h-1 w-16 bg-gradient-to-r from-purple-600 to-pink-600 mb-1"></div>
                  <h2 className="text-lg font-bold mb-2 text-gray-900" style={{ fontFamily: 'serif' }}>Eğitim</h2>
                  <div className="space-y-1">
                    {data.education.map((edu, i) => (
                      <div key={i} className="border-l-4 border-purple-600 pl-2">
                        <h3 className="font-bold text-gray-900 text-xs">{edu.degree}</h3>
                        <p className="text-purple-600 font-medium text-xs">{edu.school}</p>
                      </div>
                    ))}
                  </div>
                </section>
              ...
              {data.achievements.length > 0 && (
                <section>
                  <div className="h-1 w-16 bg-gradient-to-r from-purple-600 to-pink-600 mb-1"></div>
                  <h2 className="text-lg font-bold mb-2 text-gray-900" style={{ fontFamily: 'serif' }}>Başarılar</h2>
                  <div className="space-y-1">
