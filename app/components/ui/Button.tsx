...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const variants = {
      ...
      gradient:
        "bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 text-white focus:ring-purple-500 shadow-lg hover:shadow-xl md:hover:shadow-neon-purple",
      neon:
        "bg-gradient-to-r from-cyan-500 via-purple-500 to-pink-500 text-white focus:ring-purple-500 shadow-lg hover:shadow-xl md:hover:shadow-neon-purple border-2 border-transparent",
    };

    // Desktop-only bold hover effects
    const desktopHover = variant === "gradient" || variant === "neon"
      ? "md:hover:scale-105 md:hover:brightness-110"
      : "";
      ...
      return (
        ...
        >
          {(variant === "gradient" || variant === "neon") && (
            <span className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full md:group-hover:translate-x-full transition-transform duration-1000 pointer-events-none" />
          )}
          {content}
        </Link>
      );
