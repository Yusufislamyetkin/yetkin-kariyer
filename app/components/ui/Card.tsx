...
export const Card = forwardRef<HTMLDivElement, CardProps>(
  ({ className, variant = "default", hover = false, children, ...props }, ref) => {
    ...
    // Mobile: Simple hover, Desktop: Bold animations
    const hoverStyles = hover
      ? "cursor-pointer hover:shadow-lg md:hover:shadow-2xl md:hover:scale-[1.02] md:hover:-translate-y-1"
      : "";

    // Desktop-only glow effects
    const desktopGlow = hover && variant === "neon"
      ? "md:hover:shadow-neon-purple md:hover:border-purple-500/50"
      : "";
    ...
    return (
      ...
      >
        {variant === "neon" && (
          <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-blue-500 via-purple-500 to-pink-500 opacity-0 md:group-hover:opacity-20 transition-opacity duration-300 pointer-events-none -z-10" />
        )}
        {children}
      </div>
    );
