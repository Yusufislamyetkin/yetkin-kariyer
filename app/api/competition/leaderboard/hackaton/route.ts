...
export async function GET(request: Request) {
  try {
    ...
    const attempts = await db.hackatonAttempt.findMany({
      where: {
        completedAt: {
          gte: startDate,
          lt: endDate,
    ...
    for (const attempt of attempts) {
      ...
      const score = metrics?.projectScore || 0;
    ...
    for (const entry of leaderboardData.slice(0, 100)) {
      await db.hackatonLeaderboardEntry.upsert({
        where: {
    ...
    console.error("Error fetching hackaton leaderboard:", error);
    return NextResponse.json(
      { error: "Hackaton liderlik tablosu yüklenirken bir hata oluştu" },
      { status: 500 }
    );
  }
}
