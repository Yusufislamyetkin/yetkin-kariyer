...
export async function GET(request: Request) {
  try {
    ...
    for (const entry of leaderboardData.slice(0, 100)) {
      await db.liveCodingLeaderboardEntry.upsert({
        where: {
    ...
    console.error("Error fetching live coding leaderboard:", error);
    return NextResponse.json(
      { error: "Canlı kodlama liderlik tablosu yüklenirken bir hata oluştu" },
      { status: 500 }
    );
  }
}
