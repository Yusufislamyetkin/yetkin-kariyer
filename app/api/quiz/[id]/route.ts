  ...
  try {
    ...
    try {
      ...
      if (!userExists && session.user.email) {
        console.warn("[QUIZ_SUBMIT] Kullanıcı ID ile bulunamadı, email ile kontrol ediliyor:", {
          userId,
        ...
        if (userByEmail) {
          ...
          console.warn("[QUIZ_SUBMIT] Kullanıcı email ile bulundu ama session userId farklı, database userId kullanılıyor:", {
            sessionUserId,
      ...
      if (!userExists) {
        console.error("[QUIZ_SUBMIT] Kullanıcı veritabanında bulunamadı:", { 
          userId,
          email: session.user.email,
        });
        return NextResponse.json(
      ...
      console.error("[QUIZ_SUBMIT] Kullanıcı kontrolü hatası:", error);
      return NextResponse.json(
