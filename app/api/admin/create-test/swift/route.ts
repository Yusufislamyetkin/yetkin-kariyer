...
export async function POST() {
  try {
    ...
    console.log("[CREATE_TEST_API] Starting Swift test creation...");

    const expertise = "Swift";
    
    let testContent: any = null;
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "swift-test-modules.json";
    ...
    return NextResponse.json({
      success: true,
      message: `Swift testleri başarıyla oluşturuldu. ${modules.length} modül, ${totalTests} test eklendi.`,
      stats: {
