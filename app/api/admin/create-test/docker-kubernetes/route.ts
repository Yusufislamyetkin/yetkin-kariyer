...
export async function POST() {
  try {
    ...
    console.log("[CREATE_TEST_API] Starting Docker & Kubernetes test creation...");

    const expertise = "Docker & Kubernetes";
    
    // Test artık bağımsız - courseId opsiyonel, content field'ında modül yapısı var
    const quizId = `test-docker-kubernetes-${Date.now()}`;
    
    // Read test modules from JSON file
    let testContent: any = null;
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "docker-kubernetes-test-modules.json";
    ...
    await db.quiz.upsert({
      ...
      update: {
        ...
        description: "Docker & Kubernetes teknolojisi için test",
        type: "TEST",
        level: "intermediate",
        topic: expertise,
        questions: [],
        content: {
          ...
          overview: testContent.overview || {
            description: "Docker & Kubernetes teknolojisi için kapsamlı test paketi",
    ...
    return NextResponse.json({
      success: true,
      message: `Docker & Kubernetes test kategorisi başarıyla oluşturuldu. ${modules.length} modül eklendi.`,
      stats: {
        testsCreated: 1,
