  ...
  try {
    ...
    for (const moduleItem of modules) {
      ...
      for (const testItem of moduleItem.relatedTests) {
        ...
        // Test ID oluştur
        const testId = `test-${expertise.toLowerCase().replace(/\s+/g, '-').replace(/&/g, '')}-${moduleItem.id}-${testItem.id}`;
    ...
    return NextResponse.json({
      success: true,
      message: `Docker & Kubernetes testleri başarıyla oluşturuldu. ${modules.length} modül, ${totalTests} test eklendi.`,
      stats: {
