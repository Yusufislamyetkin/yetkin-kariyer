  ...
  try {
    ...
    if (!course) {
      course = await db.course.create({
        data: {
          title: "Docker & Kubernetes Test Kategorisi",
          description: "Docker & Kubernetes teknolojisi için test kategorisi",
          expertise,
    ...
    return NextResponse.json({
      success: true,
      message: `Docker & Kubernetes test kategorisi başarıyla oluşturuldu. Test içeriği daha sonra eklenecek.`,
      stats: {
        testsCreated: 1,
      },
    });
