  ...
  try {
    ...
    if (!course) {
      course = await db.course.create({
        data: {
          title: "React Test Kategorisi",
          description: "React teknolojisi için test kategorisi",
          expertise,
    ...
    return NextResponse.json({
      success: true,
      message: `React test kategorisi başarıyla oluşturuldu. Test içeriği daha sonra eklenecek.`,
      stats: {
        testsCreated: 1,
      },
    });
