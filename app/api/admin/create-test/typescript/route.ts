...
export async function POST() {
  try {
    ...
    console.log("[CREATE_TEST_API] Starting TypeScript test creation...");

    const expertise = "TypeScript";
    
    let testContent: any = null;
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "typescript-test-modules.json";
    ...
    return NextResponse.json({
      success: true,
      message: `TypeScript testleri başarıyla oluşturuldu. ${modules.length} modül, ${totalTests} test eklendi.`,
      stats: {
