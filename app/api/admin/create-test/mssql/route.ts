...
export async function POST() {
  try {
    ...
    console.log("[CREATE_TEST_API] Starting MSSQL test creation...");

    const expertise = "MSSQL";
    
    // Test artık bağımsız - courseId opsiyonel, content field'ında modül yapısı var
    const quizId = `test-mssql-${Date.now()}`;
    
    // Read test modules from JSON file
    let testContent: any = null;
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "mssql-test-modules.json";
    ...
    await db.quiz.upsert({
      ...
      update: {
        title: "MSSQL Test",
        description: "MSSQL teknolojisi için test",
        type: "TEST",
        level: "intermediate",
        topic: expertise,
        questions: [],
        content: {
          ...
          overview: testContent.overview || {
            description: "MSSQL teknolojisi için kapsamlı test paketi",
    ...
    return NextResponse.json({
      success: true,
      message: `MSSQL test kategorisi başarıyla oluşturuldu. ${modules.length} modül eklendi.`,
      stats: {
        testsCreated: 1,
