...
export async function POST() {
  try {
    ...
    console.log("[CREATE_TEST_API] Starting Python test creation...");

    const expertise = "Python";
    
    let testContent: any = null;
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "python-test-modules.json";
    ...
    return NextResponse.json({
      success: true,
      message: `Python testleri başarıyla oluşturuldu. ${modules.length} modül, ${totalTests} test eklendi.`,
      stats: {
