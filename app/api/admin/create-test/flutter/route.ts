...
export async function POST() {
  try {
    ...
    console.log("[CREATE_TEST_API] Starting Flutter test creation...");

    const expertise = "Flutter";
    
    // Test artık bağımsız - courseId opsiyonel, content field'ında modül yapısı var
    const quizId = `test-flutter-${Date.now()}`;
    
    // Read test modules from JSON file
    let testContent: any = null;
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "flutter-test-modules.json";
    ...
    await db.quiz.upsert({
      ...
      update: {
        title: "Flutter Test",
        description: "Flutter teknolojisi için test",
        type: "TEST",
        level: "intermediate",
        topic: expertise,
        questions: [],
        content: {
          ...
          overview: testContent.overview || {
            description: "Flutter teknolojisi için kapsamlı test paketi",
    ...
    return NextResponse.json({
      success: true,
      message: `Flutter test kategorisi başarıyla oluşturuldu. ${modules.length} modül eklendi.`,
      stats: {
        testsCreated: 1,
