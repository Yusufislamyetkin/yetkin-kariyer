...
export async function POST() {
  try {
    ...
    console.log("[CREATE_TEST_API] Starting Go test creation...");

    const expertise = "Go";
    
    let testContent: any = null;
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "go-test-modules.json";
    ...
    return NextResponse.json({
      success: true,
      message: `Go testleri başarıyla oluşturuldu. ${modules.length} modül, ${totalTests} test eklendi.`,
      stats: {
