...
export async function POST() {
  try {
    ...
    console.log("[CREATE_TEST_API] Starting NestJS test creation...");

    const expertise = "NestJS";
    
    let testContent: any = null;
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "nestjs-test-modules.json";
    ...
    return NextResponse.json({
      success: true,
      message: `NestJS testleri başarıyla oluşturuldu. ${modules.length} modül, ${totalTests} test eklendi.`,
      stats: {
