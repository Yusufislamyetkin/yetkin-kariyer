...
export async function POST() {
  try {
    ...
    console.log("[CREATE_TEST_API] Starting Kotlin test creation...");

    const expertise = "Kotlin";
    
    let testContent: any = null;
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "kotlin-test-modules.json";
    ...
    return NextResponse.json({
      success: true,
      message: `Kotlin testleri başarıyla oluşturuldu. ${modules.length} modül, ${totalTests} test eklendi.`,
      stats: {
