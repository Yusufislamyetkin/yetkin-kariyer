...
export async function POST() {
  try {
    ...
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "ethical-hacking-test-modules.json";
      ...
      update: {
        ...
        content: {
          ...
          overview: testContent.overview || {
            description: "Ethical Hacking teknolojisi için kapsamlı test paketi",
    ...
    return NextResponse.json({
      success: true,
      message: `Ethical Hacking test kategorisi başarıyla oluşturuldu. ${modules.length} modül eklendi.`,
      stats: {
        testsCreated: 1,
