...
export async function POST() {
  try {
    ...
    console.log("[CREATE_TEST_API] Starting Spring Boot test creation...");

    const expertise = "Spring Boot";
    
    let testContent: any = null;
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "spring-boot-test-modules.json";
    ...
    return NextResponse.json({
      success: true,
      message: `Spring Boot testleri başarıyla oluşturuldu. ${modules.length} modül, ${totalTests} test eklendi.`,
      stats: {
