...
export async function POST() {
  try {
    ...
    console.log("[CREATE_TEST_API] Starting MongoDB test creation...");

    const expertise = "MongoDB";
    
    let testContent: any = null;
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "mongodb-test-modules.json";
    ...
    return NextResponse.json({
      success: true,
      message: `MongoDB testleri başarıyla oluşturuldu. ${modules.length} modül, ${totalTests} test eklendi.`,
      stats: {
