...
export async function POST() {
  try {
    ...
    console.log("[CREATE_TEST_API] Starting Next.js test creation...");

    const expertise = "Next.js";
    
    // Test artık bağımsız - courseId opsiyonel, content field'ında modül yapısı var
    const quizId = `test-nextjs-${Date.now()}`;
    
    // Read test modules from JSON file
    let testContent: any = null;
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "nextjs-test-modules.json";
    ...
    await db.quiz.upsert({
      ...
      update: {
        title: "Next.js Test",
        description: "Next.js teknolojisi için test",
        type: "TEST",
        level: "intermediate",
        topic: expertise,
        questions: [],
        content: {
          ...
          overview: testContent.overview || {
            description: "Next.js teknolojisi için kapsamlı test paketi",
    ...
    return NextResponse.json({
      success: true,
      message: `Next.js test kategorisi başarıyla oluşturuldu. ${modules.length} modül eklendi.`,
      stats: {
        testsCreated: 1,
