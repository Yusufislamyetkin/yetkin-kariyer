...
export async function POST() {
  try {
    ...
    console.log("[CREATE_TEST_API] Starting PostgreSQL test creation...");

    const expertise = "PostgreSQL";
    
    let testContent: any = null;
    try {
      const fileName = TEST_MODULE_FILE_MAP[expertise] || "postgresql-test-modules.json";
    ...
    return NextResponse.json({
      success: true,
      message: `PostgreSQL testleri başarıyla oluşturuldu. ${modules.length} modül, ${totalTests} test eklendi.`,
      stats: {
