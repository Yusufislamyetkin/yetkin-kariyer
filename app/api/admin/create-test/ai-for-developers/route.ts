  ...
  try {
    ...
    if (!course) {
      course = await db.course.create({
        data: {
          title: "Yapay Zeka Test Kategorisi",
          description: "Yapay Zeka teknolojisi için test kategorisi",
          expertise,
    ...
    const quizId = `test-ai-${Date.now()}`;
    await db.quiz.upsert({
      where: { id: quizId },
      update: {
        title: "Yapay Zeka Test",
        description: "Yapay Zeka teknolojisi için test",
        type: "TEST",
        level: "intermediate",
        questions: [],
        passingScore: 60,
    ...
    return NextResponse.json({
      success: true,
      message: `Yapay Zeka test kategorisi başarıyla oluşturuldu. Test içeriği daha sonra eklenecek.`,
      stats: {
        testsCreated: 1,
      },
    });
