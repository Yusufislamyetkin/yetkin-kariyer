...
import { createTypeScriptCourse } from "@/lib/admin/create-typescript-course";
...
export async function POST() {
  try {
    ...
    console.log("[CREATE_COURSE_API] Starting TypeScript course creation...");

    // Generate course content
    let courseContent;
    try {
      courseContent = await createTypeScriptCourse();
    } catch (error: any) {
    ...
    try {
      const course = await db.course.upsert({
        where: { id: "course-typescript-roadmap" },
        create: {
          id: "course-typescript-roadmap",
          title: "TypeScript Kursu",
          description: courseContent.overview.description,
          category: "Frontend",
          field: "Software Development",
          expertise: "TypeScript", // Test oluşturma API'si için gerekli
      ...
      return NextResponse.json({
        success: true,
        message: `TypeScript kursu başarıyla oluşturuldu. ${totalModules} modül ve ${totalLessons} ders eklendi.`,
        stats: {
