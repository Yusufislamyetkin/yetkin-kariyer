...
export async function POST() {
  try {
    ...
    console.log("[CREATE_COURSE_API] Starting Flutter course creation...");

    // Read course content from JSON file
    let courseContent;
    try {
      courseContent = await createFlutterCourse();
    } catch (error: any) {
    ...
    try {
      ...
      return NextResponse.json({
        success: true,
        message: `Flutter kursu başarıyla oluşturuldu. ${totalModules} modül ve ${totalLessons} ders eklendi.`,
        stats: {
