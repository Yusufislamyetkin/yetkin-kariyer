...
import { createSpringBootCourse } from "@/lib/admin/create-spring-boot-course";
...
export async function POST() {
  try {
    ...
    console.log("[CREATE_COURSE_API] Starting Spring Boot course creation...");

    // Generate course content
    let courseContent;
    try {
      courseContent = await createSpringBootCourse();
    } catch (error: any) {
    ...
    try {
      const course = await db.course.upsert({
        where: { id: "course-spring-boot-roadmap" },
        create: {
          id: "course-spring-boot-roadmap",
          title: "Spring Boot Kursu",
          description: courseContent.overview.description,
          category: "Backend",
          field: "Software Development",
          expertise: "Spring Boot", // Test oluşturma API'si için gerekli
      ...
      return NextResponse.json({
        success: true,
        message: `Spring Boot kursu başarıyla oluşturuldu. ${totalModules} modül ve ${totalLessons} ders eklendi.`,
        stats: {
