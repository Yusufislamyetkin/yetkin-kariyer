...
import { createMongoDBCourse } from "@/lib/admin/create-mongodb-course";
...
export async function POST() {
  try {
    ...
    console.log("[CREATE_COURSE_API] Starting MongoDB course creation...");

    // Generate course content
    let courseContent;
    try {
      courseContent = await createMongoDBCourse();
    } catch (error: any) {
    ...
    try {
      const course = await db.course.upsert({
        where: { id: "course-mongodb-roadmap" },
        create: {
          id: "course-mongodb-roadmap",
          title: "MongoDB Kursu",
          description: courseContent.overview.description,
          category: "Database",
          field: "Software Development",
          expertise: "MongoDB", // Test oluşturma API'si için gerekli
      ...
      return NextResponse.json({
        success: true,
        message: `MongoDB kursu başarıyla oluşturuldu. ${totalModules} modül ve ${totalLessons} ders eklendi.`,
        stats: {
