...
import { createGoCourse } from "@/lib/admin/create-go-course";
...
export async function POST() {
  try {
    ...
    console.log("[CREATE_COURSE_API] Starting Go course creation...");

    // Generate course content
    let courseContent;
    try {
      courseContent = await createGoCourse();
    } catch (error: any) {
    ...
    try {
      const course = await db.course.upsert({
        where: { id: "course-go-roadmap" },
        create: {
          id: "course-go-roadmap",
          title: "Go Kursu",
          description: courseContent.overview.description,
          category: "Backend",
          field: "Software Development",
          expertise: "Go", // Test oluşturma API'si için gerekli
      ...
      return NextResponse.json({
        success: true,
        message: `Go kursu başarıyla oluşturuldu. ${totalModules} modül ve ${totalLessons} ders eklendi.`,
        stats: {
