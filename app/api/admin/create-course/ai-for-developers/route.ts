...
export async function POST() {
  try {
    ...
    console.log("[CREATE_COURSE_API] Starting AI for Developers course creation...");

    // Generate course content from JSON file
    let courseContent;
    try {
      courseContent = await createAICourse();
    } catch (error: any) {
    ...
    try {
      const course = await db.course.upsert({
        where: { id: "course-ai-for-developers" },
        create: {
          id: "course-ai-for-developers",
          title: "Yazılımcılar İçin Yapay Zeka",
      ...
      return NextResponse.json({
        success: true,
        message: `Yazılımcılar İçin Yapay Zeka kursu başarıyla oluşturuldu. ${totalModules} modül ve ${totalLessons} ders eklendi.`,
        stats: {
