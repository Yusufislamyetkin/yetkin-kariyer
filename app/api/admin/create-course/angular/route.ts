...
export async function POST() {
  try {
    ...
    console.log("[CREATE_COURSE_API] Starting Angular course creation...");

    // Generate course content using AI
    let courseContent;
    try {
      courseContent = await createAngularCourse();
    } catch (error: any) {
    ...
    try {
      const course = await db.course.upsert({
        where: { id: "course-angular-roadmap" },
        create: {
          id: "course-angular-roadmap",
          title: "Angular Kursu",
      ...
      return NextResponse.json({
        success: true,
        message: `Angular kursu başarıyla oluşturuldu. ${totalModules} modül ve ${totalLessons} ders eklendi.`,
        stats: {
