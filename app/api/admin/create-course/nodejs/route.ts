...
export async function POST() {
  try {
    ...
    console.log("[CREATE_COURSE_API] Starting Node.js course creation...");

    // Generate course content using AI
    let courseContent;
    try {
      courseContent = await createNodeJSCourse();
    } catch (error: any) {
    ...
    try {
      const course = await db.course.upsert({
        where: { id: "course-nodejs-roadmap" },
        create: {
          id: "course-nodejs-roadmap",
          title: "Node.js Kursu",
          description: courseContent.overview.description,
          category: "Backend",
          field: "Software Development",
          expertise: "Node.js", // Test oluşturma API'si için gerekli
      ...
      return NextResponse.json({
        success: true,
        message: `Node.js kursu başarıyla oluşturuldu. ${totalModules} modül ve ${totalLessons} ders eklendi.`,
        stats: {
