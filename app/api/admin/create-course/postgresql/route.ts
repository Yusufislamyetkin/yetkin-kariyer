...
import { createPostgreSQLCourse } from "@/lib/admin/create-postgresql-course";
...
export async function POST() {
  try {
    ...
    console.log("[CREATE_COURSE_API] Starting PostgreSQL course creation...");

    // Generate course content
    let courseContent;
    try {
      courseContent = await createPostgreSQLCourse();
    } catch (error: any) {
    ...
    try {
      const course = await db.course.upsert({
        where: { id: "course-postgresql-roadmap" },
        create: {
          id: "course-postgresql-roadmap",
          title: "PostgreSQL Kursu",
          description: courseContent.overview.description,
          category: "Database",
          field: "Database Management",
          expertise: "PostgreSQL", // Test oluşturma API'si için gerekli
      ...
      return NextResponse.json({
        success: true,
        message: `PostgreSQL kursu başarıyla oluşturuldu. ${totalModules} modül ve ${totalLessons} ders eklendi.`,
        stats: {
