...
import { createNestJSCourse } from "@/lib/admin/create-nestjs-course";
...
export async function POST() {
  try {
    ...
    console.log("[CREATE_COURSE_API] Starting NestJS course creation...");

    // Generate course content
    let courseContent;
    try {
      courseContent = await createNestJSCourse();
    } catch (error: any) {
    ...
    try {
      const course = await db.course.upsert({
        where: { id: "course-nestjs-roadmap" },
        create: {
          id: "course-nestjs-roadmap",
          title: "NestJS Kursu",
          description: courseContent.overview.description,
          category: "Backend",
          field: "Software Development",
          expertise: "NestJS", // Test oluşturma API'si için gerekli
      ...
      return NextResponse.json({
        success: true,
        message: `NestJS kursu başarıyla oluşturuldu. ${totalModules} modül ve ${totalLessons} ders eklendi.`,
        stats: {
