...
import { createAWSCourse } from "@/lib/admin/create-aws-course";
...
export async function POST() {
  try {
    ...
    console.log("[CREATE_COURSE_API] Starting AWS course creation...");

    // Generate course content
    let courseContent;
    try {
      courseContent = await createAWSCourse();
    } catch (error: any) {
    ...
    try {
      const course = await db.course.upsert({
        where: { id: "course-aws-roadmap" },
        create: {
          id: "course-aws-roadmap",
          title: "AWS Kursu",
          description: courseContent.overview.description,
          category: "DevOps",
          field: "Cloud Computing",
          expertise: "AWS", // Test oluşturma API'si için gerekli
      ...
      return NextResponse.json({
        success: true,
        message: `AWS kursu başarıyla oluşturuldu. ${totalModules} modül ve ${totalLessons} ders eklendi.`,
        stats: {
