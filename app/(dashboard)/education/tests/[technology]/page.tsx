...
import { ChevronLeft, FileText, Clock, ArrowRight, Code2, Compass, BookOpen } from "lucide-react";
  ...
  useEffect(() => {
    ...
    const fetchModules = async () => {
      ...
      try {
        ...
        setTechnologyName(data.technology || decodedTechnology);
      } catch (err) {
            ...
            <div className="grid gap-4 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {modules.map((module, index) => {
                return (
                  <Link
                    key={module.id}
                    href={`/education/tests/${encodeURIComponent(decodedTechnology)}/${encodeURIComponent(module.id)}`}
