...
export default function QuizDetailPage() {
  ...
  // Technology name'i decode et - Utility fonksiyonunu kullan
  // Route'tan gelen technology zaten normalize edilmiş format (tire ile ayrılmış)
  ...
  const fetchQuiz = async () => {
    ...
    try {
      const response = await fetch(
        `/api/education/tests/quiz/${encodeURIComponent(technology || "")}/${encodeURIComponent(moduleId || "")}/${encodeURIComponent(quizId || "")}/questions`
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/education/tests/quiz/${encodeURIComponent(technology || "")}/${encodeURIComponent(moduleId || "")}/${encodeURIComponent(quizId || "")}`,
        {
          method: "POST",
  ...
  // Technology name'i route formatına çevir (geri dönüş için)
  // Utility fonksiyonunu kullan
  const technologyRoute = technologyToRoute(decodedTechnology);
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <Card variant="elevated" className="p-8 text-center max-w-md">
          ...
          <Link href={`/education/tests/${technologyRoute}/modules/${encodeURIComponent(moduleId || "")}`}>
