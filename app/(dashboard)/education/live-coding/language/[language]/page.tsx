...
import type { LiveCodingQuizSummary } from "@/types/live-coding";
...
> = {
  csharp: {
    label: "C# Canlı Kodlama",
    description:
      "Console çıktıları, koleksiyonlar ve mini .NET projeleriyle backend yetkinliğinizi güçlendirin.",
    badge: ".NET & Console",
    accent: "from-violet-600 via-indigo-500 to-sky-500",
  },
  javascript: {
    label: "JavaScript Görevleri",
    description:
      "ESNext özellikleri, döngüler ve DOM odaklı görevlerle modern web becerilerinizi artırın.",
    badge: "Frontend & Web",
    accent: "from-amber-500 via-orange-500 to-yellow-500",
  },
  python: {
    label: "Python Atölyesi",
    description:
      "Veri yapıları, fonksiyonlar ve otomasyon scriptleri üzerinden Python pratiği yapın.",
    badge: "Data & Automation",
    accent: "from-emerald-500 via-green-500 to-teal-500",
  },
  java: {
    label: "Java Sprintleri",
    description:
      "Collection framework, stream API ve OOP prensipleriyle kurumsal Java görevlerini çözün.",
    badge: "Backend & OOP",
...
export default function LiveCodingLanguagePage() {
  ...
  const [items, setItems] = useState<LiveCodingQuizSummary[]>([]);
  ...
  // Filters'ı stringify edip karşılaştır - object referans değişikliğini önle
  ...
  }, [filters.expertise, filters.topic, filters.content, filters.level]);

  const fetchItems = useCallback(async () => {
    if (!resolvedLanguage) return;

    // Rate limiting: Son 5 saniye içinde fetch yapıldıysa skip et (artırıldı)
    const now = Date.now();
    if (now - lastFetchRef.current < 5000) {
      console.log("[LiveCoding] Rate limit: Skipping fetch");
      return;
    }

    // Eğer filters ve search değişmediyse skip et
    if (
      filtersKey === lastFiltersRef.current &&
      debouncedSearch === lastDebouncedSearchRef.current &&
      lastFetchRef.current > 0
    ) {
      console.log("[LiveCoding] No changes detected, skipping fetch");
      return;
    }
    ...
    try {
      ...
      }, 10000); // Reduced timeout to 10 seconds for faster failure detection

      const params = new URLSearchParams();
      params.append("type", "LIVE_CODING");
      ...
      let data: { items?: LiveCodingQuizSummary[] } = {};
      ...
      try {
        ...
        if (!response.ok) {
          // Error durumunda retry yapma, direkt hata göster
          // Retry mekanizması infinite loop'a neden olabilir
          throw new Error(json?.error || "Canlı kodlamalar yüklenirken bir hata oluştu. Lütfen sayfayı yenileyin.");
        }

        // Başarılı fetch'te retry sayacını sıfırla
      ...
      const filteredItems = Array.isArray(data.items)
        ? data.items.filter(
            (candidate): candidate is LiveCodingQuizSummary =>
              typeof candidate?.id === "string" && candidate?.questions !== undefined
          )
        : [];

      // Log for debugging
      console.log(`[LiveCoding] Fetched ${data.items?.length || 0} items, filtered to ${filteredItems.length} valid items`);
      ...
      console.error("Error fetching live coding items:", fetchError);
      
      // Retry mekanizması kaldırıldı - infinite loop riski
      // Kullanıcı manuel olarak "Tekrar dene" butonuna basabilir
      if ((fetchError as Error).name === "AbortError") {
        setError("İstek zaman aşımına uğradı. Lütfen tekrar deneyin.");
      } else {
        setError("Canlı kodlamalar yüklenirken bir hata oluştu. Lütfen daha sonra tekrar deneyin.");
      }
  ...
  }, [filtersKey, debouncedSearch, resolvedLanguage]);

  // Filters veya search değiştiğinde fetch yap - ama debounce ile
  useEffect(() => {
    if (!resolvedLanguage) return;

    // Debounce: Filters değişikliklerinde de bekle
    const timeoutId = setTimeout(() => {
      fetchItems();
    }, 800); // 800ms debounce - search'ten daha uzun
  ...
  }, [filtersKey, debouncedSearch, resolvedLanguage, fetchItems]);
  ...
  const challenges = useMemo<ChallengeCard[]>(() => {
    ...
    return items
      .map((item) => {
        // Use pre-normalized data from API if available, otherwise fallback to client-side normalization
        let tasks: LiveCodingTask[] = [];
        
        if (item.liveCoding?.tasks) {
          // Use server-side normalized data (optimized path)
          tasks = item.liveCoding.tasks;
        } else if (item.questions) {
          // Fallback to client-side normalization for backward compatibility
      ...
      <div className="flex min-h-[400px] items-center justify-center">
        <Card variant="elevated" className="max-w-md p-8 text-center">
          ...
          <p className="mt-2 text-sm text-gray-400">
            Seçtiğiniz programlama dili için canlı kodlama içerikleri bulunamadı. Lütfen mevcut dillerden birini seçin.
          </p>
          <Link href="/education/live-coding" className="mt-6 inline-block">
      ...
      <section className="space-y-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex-1">
            <Input
              placeholder="Görev veya içerik ara..."
              value={search}
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              className={cn(
                filters.level === null && "bg-gradient-to-r from-emerald-600 to-teal-600 text-white"
              )}
            >
              Tümü
            </Button>
      ...
      {loading ? (
        <div className="flex min-h-[280px] items-center justify-center">
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-gray-400">Canlı kodlama görevleri yükleniyor...</p>
          </div>
        </div>
      ) : error ? (
        <Card variant="elevated">
          <CardContent className="py-16 text-center space-y-3">
            <Code className="mx-auto h-12 w-12 text-red-400" />
        ...
        <Card variant="elevated">
          <CardContent className="py-16 text-center space-y-3">
            ...
            <h3 className="text-lg font-semibold text-gray-100">Görev bulunamadı</h3>
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {challenges.map(({ item, tasks, highlights }, index) => (
            <Card
              ...
              variant="elevated"
              hover
              className="group border border-gray-900/60 bg-gray-950/80 backdrop-blur transition duration-200 overflow-hidden relative"
              style={{ animationDelay: `${index * 0.07}s` }}
            >
              <Link 
                href={`/education/live-coding/${item.id}`}
                className="block h-full w-full no-underline text-inherit hover:text-inherit cursor-pointer rounded-xl"
              >
                <CardHeader>
                ...
                <CardContent className="space-y-4 border-t border-gray-900/80 pt-4">
                  <div className="flex flex-wrap gap-2 text-xs font-semibold text-gray-300">
                    ...
                    <span className="rounded bg-slate-900 px-2 py-1 uppercase tracking-wide text-emerald-300">
