...
const LANGUAGE_CARDS: LiveCodingLanguageMeta[] = [
  {
    language: "csharp",
    title: "C# Canlı Kodlama",
    description: "Console.WriteLine, döngüler ve mini projelerle .NET ekosisteminde uzmanlaşın.",
    badge: ".NET & OOP",
    icon: Hexagon,
  ...
  {
    language: "javascript",
    title: "JavaScript Görevleri",
    description: "DOM manipülasyonu, fonksiyonel programlama ve asenkron kodlama pratikleri.",
    badge: "Web & Frontend",
    icon: Globe2,
  ...
  },
  {
    ...
    description: "List comprehension, veri analizi ve algoritma görevleriyle kendinizi geliştirin.",
    badge: "Data & AI",
    icon: Layers,
  ...
  {
    language: "java",
    title: "Java Sprintleri",
    description: "Collection framework, stream API ve mini backend görevleriyle pratik yapın.",
    badge: "Backend & API",
    icon: Cpu,
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <header className="space-y-3">
        <span className="inline-flex items-center gap-2 rounded-full bg-emerald-500/10 px-3 py-1 text-xs font-semibold uppercase tracking-wide text-emerald-600 dark:text-emerald-300">
          <TerminalSquare className="h-3.5 w-3.5" />
          Live Coding Studio
        </span>
        <h1 className="text-4xl font-bold text-gray-900 dark:text-gray-100">
          Programlama Dillerinde Canlı Kodlama
        </h1>
        <p className="max-w-2xl text-sm text-gray-600 dark:text-gray-400">
          Adım adım ilerleyen canlı kodlama görevleriyle pratiğe başlayın. Dilediğiniz programlama dilini seçin,
          hazıra getirilmiş öğrenme yolculuğuna hemen katılın.
        </p>
      </header>
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        {LANGUAGE_CARDS.map(({ language, title, description, badge, icon: Icon, gradient }) => (
          ...
          >
            ...
            <Link href={`/education/live-coding/language/${language}`} className="relative z-10 block h-full">
              ...
              <CardContent className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-sm font-semibold text-white/90 shadow-lg transition-all duration-200 group-hover:border-white/30 group-hover:bg-black/30 group-hover:shadow-xl">
                ...
                <Code className="h-4 w-4 transition-transform duration-200 group-hover:translate-x-1.5" />
              </CardContent>
            </Link>
          </Card>
        ))}
