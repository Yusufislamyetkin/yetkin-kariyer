...
export default function DashboardPage() {
  ...
  const recentlyCompletedGoals = dailyGoals.filter((goal) => goal.completed).slice(0, 3);
  ...
  const fetchCoreData = async () => {
    try {
      ...
      const [strikeResponse, statsResponse, interviewResponse, goalsResponse, badgesResponse, dailyRankResponse, monthlyRankResponse, activitiesResponse] = await Promise.all([
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {(recentBadges.length > 0 || recentlyCompletedGoals.length > 0) && (
              ...
              <div className="space-y-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {recentlyCompletedGoals.map((goal) => (
                    <div
                      key={goal.id}
                      className="flex items-start gap-3 rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 px-3 py-3"
                    >
                      <CheckCircle className="h-5 w-5 text-emerald-500 flex-shrink-0 mt-1" />
                      <div>
                        <p className="text-sm font-semibold text-blue-800 dark:text-blue-200">
                          {goal.goalType === "test_count" && "Test Çözme Hedefi"}
                          {goal.goalType === "score_target" && "Skor Hedefi"}
                          {goal.goalType === "topic_complete" && "Konu Tamamlama Hedefi"}
                          {goal.goalType === "streak_maintain" && "Streak Hedefi"}
                        </p>
      ...
      {/* Daily Goals */}
      {dailyGoals.length > 0 && (
        <Card variant="elevated" hover className="mt-4">
